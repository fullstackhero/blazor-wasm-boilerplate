...
internal class AzureAdClaimsPrincipalFactory : AccountClaimsPrincipalFactory<RemoteUserAccount>
{
    ...
    public override async ValueTask<ClaimsPrincipal> CreateUserAsync(RemoteUserAccount account, RemoteAuthenticationUserOptions options)
    {
        ...
        if (principal.Identity?.IsAuthenticated is true)
        {
            ...
            if (!userIdentity.HasClaim(c => c.Type == FLClaims.Fullname))
            {
                userIdentity.AddClaim(new Claim(FLClaims.Fullname, $"{userDetails.FirstName} {userDetails.LastName}"));
            }
            ...
            if (!string.IsNullOrWhiteSpace(userDetails.ImageUrl) && !userIdentity.HasClaim(c => c.Type == FLClaims.ImageUrl) && userDetails.ImageUrl is not null)
            {
                userIdentity.AddClaim(new Claim(FLClaims.ImageUrl, userDetails.ImageUrl));
            }

            var permissions = await _services.GetRequiredService<IPersonalClient>().GetPermissionsAsync();

            userIdentity.AddClaims(permissions.Select(permission => new Claim(FLClaims.Permission, permission)));
        }
